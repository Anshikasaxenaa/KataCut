 ...
 */
export function calculateDormancy(subscription: Partial<Subscription>): { status: SubscriptionStatus, dormantDays: number } {
  ...
  if (status !== 'dormant' && ['monthly', 'weekly', 'unknown'].includes(subscription.frequency) && daysSinceLast > 60) {
    status = 'dormant';
  }
  ...
  if (status === 'active' && subscription.nextBilling) {
    const now = new Date();
    const daysUntilNext = Math.round((subscription.nextBilling.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
