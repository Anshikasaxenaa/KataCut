...
function isNonSubscription(group: NormalizedTransaction[]): boolean {
  ...
  if (avgAmount > 8000 && avgAmount % 500 === 0) {
    const isTransfer = group.some(t => t.rawDescription.includes("IMPS") || t.rawDescription.includes("NEFT"));
    if (isTransfer) return true;
  }
  
  // EMI and SIP keywords
  const filterKeywords = ["EMI", "LOAN", "SIP", "MUTUAL FUND", "SALARY", "ATM", "CASH", "WITHDRAWAL"];
 ...
 */
export function detectRecurrence(merchant: string, group: NormalizedTransaction[]): Partial<Subscription> | null {
  ...
  else if (averageInterval >= 80 && averageInterval <= 100) frequency = 'quarterly';
  else if (averageInterval >= 350 && averageInterval <= 380) frequency = 'yearly';
  ...
  const knownMatch = KNOWN_MERCHANTS_DB.find(km => km.merchant.toLowerCase() === merchant.toLowerCase());
  ...
  if (variance <= 5) confidence += 30; // High amount consistency
  else if (variance <= 15) confidence += 10; // Medium consistency
  ...
  const { nextBilling, stdDev } = predictNextBilling(sorted[sorted.length - 1].date, averageInterval, intervals);
