...
export function parseHDFC(text: string): RawTransaction[] {
  ...
  // Regex to match a standard HDFC transaction line start
  // Matches: Date in DD/MM/YY format at the beginning of the line
  // Example string: "15/07/26 UPI-ZOMATO-PAYTM..."
  // ^       : Start of string
  // (\d{2}) : 2 digits for day
  // \/      : Literal slash
  // (\d{2}) : 2 digits for month
  // \/      : Literal slash
  // (\d{2}) : 2 digits for year
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (dateMatch) {
      ...
      // Example string: "15/07/26 UPI-ZOMATO 123456 15/07/26 499.00"
      // ^\d{2}\/\d{2}\/\d{2}\s+ : Date at start followed by spaces
      // (.*?)                   : Non-greedy match for the narration text (Capture Group 1)
      // (?:\s+\d{6,})?          : Optional ref number (6+ digits)
      // (?:\s+\d{2}\/\d{2}\/\d{2})? : Optional value date
      // (?:\s+[\d,]+\.\d{2})    : Matches the first amount (e.g., 499.00)
      const narrationMatch = line.match(
        /^\d{2}\/\d{2}\/\d{2}\s+(.*?)(?:\s+\d{6,})?(?:\s+\d{2}\/\d{2}\/\d{2})?(?:\s+[\d,]+\.\d{2})/,
      );
