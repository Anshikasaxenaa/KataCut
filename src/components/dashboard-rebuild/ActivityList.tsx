import { MoreVertical, FileText, CheckCircle2 } from "lucide-react";

export function ActivityList() {
  return (
    <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-[0_2px_20px_rgb(0,0,0,0.04)] h-full flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-zinc-900">Today's Activity List</h2>
        <button className="text-zinc-400 hover:text-zinc-600">
          <MoreVertical className="w-5 h-5" />
        </button>
      </div>

      <div className="flex-1 space-y-6">
        
        {/* Item 1 */}
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center shrink-0 mt-1">
            <FileText className="w-5 h-5 text-zinc-500" />
          </div>
          <div>
            <h3 className="font-semibold text-zinc-900">Review Tasks</h3>
            <p className="text-sm text-zinc-500 mt-1">Check tasks and update progress status</p>
          </div>
        </div>

        {/* Item 2 - Active */}
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-full bg-[#0066FF] flex items-center justify-center shrink-0 mt-1 shadow-md shadow-[#0066FF]/20">
            <FileText className="w-5 h-5 text-white" />
          </div>
          <div className="flex-1">
            <h3 className="font-semibold text-zinc-900">Team Sync Meeting</h3>
            <p className="text-sm text-zinc-500 mt-1">Align team on priorities and daily goals</p>
          </div>
          <div className="shrink-0 mt-2">
            <div className="w-6 h-6 rounded-full bg-[#00d284] text-white flex items-center justify-center">
              <CheckCircle2 className="w-4 h-4 border-none" />
            </div>
          </div>
        </div>

        {/* Item 3 */}
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center shrink-0 mt-1">
            <FileText className="w-5 h-5 text-zinc-500" />
          </div>
          <div>
            <h3 className="font-semibold text-zinc-900">Client Communication</h3>
            <p className="text-sm text-zinc-500 mt-1">Respond to client emails and messages</p>
          </div>
        </div>

        {/* Item 4 */}
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center shrink-0 mt-1">
            <FileText className="w-5 h-5 text-zinc-500" />
          </div>
          <div>
            <h3 className="font-semibold text-zinc-900">Project Updates</h3>
            <p className="text-sm text-zinc-500 mt-1">Update timelines tasks and milestone</p>
          </div>
        </div>

      </div>
    </div>
  );
}
